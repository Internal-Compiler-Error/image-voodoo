...
{
    match result {
        MinMaxResult::OneElement(min_max) => (*min_max, *min_max),
...
pub fn rotate_rad(image: &CanvasImage, radian: f64) -> CanvasImage {
    ...
    let (cx, cy) = (
        image.horizontal_size() as f64 / 2f64,
        image.vertical_size() as f64 / 2f64,
    );
    ...
    let rgba = iproduct!(0..new_height, 0..new_width).flat_map(|(y, x)| {
        ...
        let pixel = if x >= 0
            && x < image.horizontal_size() as i32
            && y >= 0
            && y < image.vertical_size() as i32
        {
...
fn width_height_after_rotation(radian: f64, width: f64, height: f64) -> (u32, u32) {
    let (new_width, new_height) = {
        ...
        let x_minmax = rotated_corners.clone().map(|(x, _)| x).minmax();
        let y_minmax = rotated_corners.clone().map(|(_, y)| y).minmax();
...
fn get_operation_matrix(
    ...
    let (new_width, new_height) =
        new_size_after_transformation(width + 1., height + 1., transformation);

    // offset from the center before rotation
    let (cx, cy) = (width as f64 / 2f64, height as f64 / 2f64);
    ...
    let to_after = Matrix3::new(1.0, 0.0, cx, 0.0, 1.0, cy, 0.0, 0.0, 1.0);

    let to_before = Matrix3::new(1.0, 0.0, -cx - ox, 0.0, 1.0, -cy - oy, 0.0, 0.0, 1.0);
...
fn rotate_via_matrix(image: &CanvasImage, radian: f64) -> CanvasImage {
    ...
    let rotate = Matrix2::new(radian.cos(), -radian.sin(), radian.sin(), radian.cos());
    let transform = get_operation_matrix(
        image.horizontal_size() as usize - 1,
...
fn shear(image: &CanvasImage, lambda: f64, miu: f64) -> CanvasImage {
    ...
    let shear = Matrix2::new(1.0 + lambda * miu, lambda, miu, 1.0);
...
fn new_size_after_transformation(
    horizontal_size: f64,
    vertical_size: f64,
    transformation: &Matrix2<f64>,
) -> (u32, u32) {
    ...
    let x_minmax = transformed_corners.clone().map(|(x, _)| x).minmax();
    let y_minmax = transformed_corners.clone().map(|(_, y)| y).minmax();
...
fn width_height_after_rotation_matrix(
    radian: f64,
    horizontal_size: f64,
    vertical_size: f64,
) -> (u32, u32) {
    let rotation_matrix = Matrix2::new(radian.cos(), -radian.sin(), radian.sin(), radian.cos());
